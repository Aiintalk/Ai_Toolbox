...
访问地址：`http://localhost:3000`（部署时 basePath 为 `/seeding-writer`）

## 7. 开发注意事项

- `basePath` 为 `/seeding-writer`，接口调用路径以此为前缀（部署时）
- 种草素材库中的素材类型默认为「种草文案」，与千川素材库在同一目录结构下
- `parse-product` 接口依赖 `mammoth` 库解析 Word 文档，部署前需确认已安装
...
- **完成度**：核心功能完整，已上线
- **已知问题**：
  - **`mammoth` 依赖**：DOCX 解析在未安装 `mammoth` 时会 crash，部署前必须确认
  - OSS 临时文件不清理
  - 无身份验证
...
| 步骤 | 测试项 | 操作说明 | 预期结果 | 状态 |
|------|--------|----------|----------|------|
| Step 1 | 人设列表加载 | 进入页面，查看人设列表 | 列表显示所有可用人设 | ⬜ |
| Step 1 | 选择人设 | 点击任意人设 | 成功加载 soul.md 和内容规划 | ⬜ |
| Step 2 | 上传产品文档 | 上传 PDF / Word / Excel 格式文件 | AI 返回种草向卖点提炼结果 | ⬜ |
| Step 2 | 卖点迭代 | 对提炼结果追加需求 | AI 重新生成卖点 | ⬜ |
| Step 2 | 确认卖点 | 确认后进入下一步 | 成功进入 Step 3 | ⬜ |
| Step 3 | 视频链接输入 | 输入抖音种草视频分享链接 | 成功抓取视频信息 | ⬜ |
| Step 3 | 视频转录 | 等待转录完成 | 返回完整文案文本 | ⬜ |
| Step 3 | 结构分析 | 查看 AI 结构分析 | 显示视频内容结构拆解 | ⬜ |
| Step 3 | 主题选择 | 选择主题：沿用 / 自定义 / AI 推荐 | 主题设置成功 | ⬜ |
| Step 4 | 脚本生成 | 等待 AI 生成种草脚本 | 流式输出种草风格脚本 | ⬜ |
| Step 4 | 多轮迭代 | 追加需求修改脚本 | AI 根据需求重新生成 | ⬜ |
