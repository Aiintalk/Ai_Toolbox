...
3. 字数控制——${transcriptLength > 0 ? `原视频 ${transcriptLength} 字，不能超过这个数。宁可少10%，绝不多1%` : '千川常规 250-350 字，不超过 350 字'}
...
从【卖点1】开始写，不要写【开头】。`
    ...
    try {
      ...
      // 后置字数校验
      const targetMax = transcriptLength > 0 ? transcriptLength : 350
      setLoading('校验字数...')
...
3. 字数控制——${transcriptLen > 0 ? `原视频 ${transcriptLen} 字，不能超过，宁短勿长` : '千川常规不超过 350 字'}
...
员工说哪里不对就改哪里，不动没问题的部分。每次输出完整脚本+自检表+原创度自检。不要提问，直接改。`
    ...
    try {
      ...
      // 后置字数校验
      const iterTargetMax = transcriptLen > 0 ? transcriptLen : 350
      setLoading('校验字数...')
